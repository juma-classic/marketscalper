...
export const AnalysisTool: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'dashboard' | 'patterns' | 'statistics' | 'export'>('dashboard');
    ...
    useEffect(() => {
        if (tickHistory.length >= 20) {
            const lastDigits = getLastDigits(50);
            const prediction = analysisAIService.analyzePrediction(lastDigits, selectedMarket);
            setAiPrediction(prediction);
        }
    }, [tickHistory.length, selectedMarket, getLastDigits]);

    // Calculate advanced statistics
    const calculateAdvancedStats = () => {
        const lastDigits = getLastDigits(parseInt(timeRange));
    ...
    const exportData = (format: 'csv' | 'json') => {
        const data = tickHistory.map(tick => ({
                        ...
                        <div className='sidebar-section'>
                            ...
                            <div className='quick-stats'>
                                <div className='stat-item'>
                                    ...
                                    <span className='stat-value'>{currentTick?.quote.toFixed(2) || '--'}</span>
                                </div>
                                <div className='stat-item'>
                                ...
                                <div className='stat-item'>
                                    ...
                                    <span className='stat-value hot'>{getHotDigits(3).join(', ')}</span>
                                </div>
                                <div className='stat-item'>
                                    <span className='stat-label'>Cold Digits:</span>
                                    <span className='stat-value cold'>{getColdDigits(3).join(', ')}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div className='dashboard-card live-market-card'>
                                    ...
                                    <div className='market-stats-grid'>
                                        <div className='market-stat'>
                                            ...
                                            <span className='stat-value-large'>{currentTick?.quote.toFixed(2) || '--'}</span>
                                        </div>
                                        <div className='market-stat'>
                                            <span className='stat-label'>Last Digit</span>
                                            <span className='stat-value-large highlight'>{currentTick?.lastDigit ?? '--'}</span>
                                        </div>
                                        <div className='market-stat'>
                                            <span className='stat-label'>Total Ticks</span>
                                        ...
                                        <div className='market-stat'>
                                            ...
                                            <span className={`stat-value-large ${isSubscribed ? 'connected' : 'disconnected'}`}>
                                            ...
                                            <div className='prediction-main'>
                                                ...
                                                <div className='prediction-details'>
                                                    <div className='detail-item'>
                                                        ...
                                                        <span className='value'>{aiPrediction.recommendedStrategy.toUpperCase()}</span>
                                                    </div>
                                                    <div className='detail-item'>
                                                        <span className='label'>Win Rate:</span>
                                                        <span className='value'>{aiPrediction.expectedWinRate.toFixed(1)}%</span>
                                                    </div>
                                ...
                                <div className='dashboard-card heatmap-card'>
                                    ...
                                    <div className='heatmap-grid'>
                                        {digitStats.map(stat => {
                                            ...
                                            const isHot = getHotDigits(3).includes(stat.digit);
                                            const isCold = getColdDigits(3).includes(stat.digit);
                                            return (
                                                <div 
                                                    key={stat.digit} 
                                    ...
                                    <div className='heatmap-legend'>
                                        <span>🔥 Hot: {getHotDigits(3).join(', ')}</span>
                                        <span>❄️ Cold: {getColdDigits(3).join(', ')}</span>
                                    </div>
                                </div>
                                    ...
                                    <div className='trend-content'>
                                        <div className='trend-bars'>
                                            <div className='trend-bar'>
                                                ...
                                                <div className='bar-container'>
                                                    ...
                                                    >
                                                        <span className='bar-text'>{advancedStats.evenPercentage.toFixed(0)}%</span>
                                                    </div>
                                                </div>
                                            ...
                                            <div className='trend-bar'>
                                                ...
                                                <div className='bar-container'>
                                                    ...
                                                    >
                                                        <span className='bar-text'>{advancedStats.oddPercentage.toFixed(0)}%</span>
                                                    </div>
                                                </div>
                                        ...
                                        {advancedStats.evenPercentage > 60 && (
                                            <div className='trend-alert'>
                                                🔔 <strong>Alert:</strong> Strong EVEN dominance! Consider ODD reversal strategy.
                                            </div>
                                        )}
                                        {advancedStats.oddPercentage > 60 && (
                                            <div className='trend-alert'>
                                                🔔 <strong>Alert:</strong> Strong ODD dominance! Consider EVEN reversal strategy.
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className='overview-grid'>
                                    ...
                                    <div className='chart-card'>
                                        ...
                                        <div className='digit-bars'>
                                            {digitStats.map(stat => (
                                                <div key={stat.digit} className='digit-bar-item'>
                                                    <div className='bar-container'>
                                                        <div
                                                            className='bar-fill'
                                                            style={{
                                    ...
                                    <div className='chart-card'>
                                        ...
                                        <div className='recent-digits'>
                                            {getLastDigits(20).map((digit, index) => (
