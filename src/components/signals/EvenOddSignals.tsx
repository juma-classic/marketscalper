...
export const EvenOddSignals: React.FC<{ onSignalsUpdate?: (signals: EvenOddSignal[]) => void }> = ({
    ...
    const analyzeEvenOdd = (
        ...
        // CONSERVATIVE: Imbalance >65% + Streak 5+ + Digit Analysis aligned
        if (imbalance >= 15) {
            // 65% vs 35% = 30% difference, but we use 15% from 50% baseline
        ...
        // AGGRESSIVE: Imbalance >55% OR Streak 4+
        if (!signalType) {
            if (imbalance >= 5) {
                // 55% vs 45% = 10% difference, but we use 5% from 50% baseline
                const underrepresented = evenPercentage < oddPercentage ? 'EVEN' : 'ODD';
                signalType = underrepresented;
    ...
    const scanAllMarkets = async () => {
        setIsScanning(true);
        console.log('🎲 EVEN/ODD Scanner: Scanning all markets...');
        ...
        setIsScanning(false);
        console.log('✅ EVEN/ODD Scanner: Scan complete');
    };
    ...
    return (
        <div className='evenodd-signals'>
            ...
            <div className='evenodd-signals-list'>
                ...
                {signals.map(signal => (
                    <div key={signal.id} className='signal-card evenodd-signal-card'>
                            ...
                            >
                                ...
                                {signal.entryAnalysis.suggestedEntry !== 'WAIT' && (
                                    <div className='suggestion-timing'>
                                        ...
                                        {signal.entryAnalysis.timing === 'WAIT_2_TICKS' && '⏱️ Wait 2 Ticks'}
                                    </div>
                                )}
                            </div>
                        )}
