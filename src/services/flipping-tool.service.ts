...
class FlippingToolService {
     ...
     */
    async start(config: FlippingToolConfig, callbacks?: {
    ...
    private async subscribeToMarket(): Promise<void> {
        ...
        try {
            this.tickSubscriptionId = await derivAPIService.subscribeToTicks(
                this.config.market,
                (tickData) => {
                    if (this.isRunning) {
     ...
     */
    private async handleTick(tickData: any): Promise<void> {
        if (!this.config || !this.isRunning) return;

        const lastDigit = this.getLastDigit(tickData.tick.quote);
        
        // Check if we should place a trade based on the strategy
        const shouldTrade = this.shouldPlaceTrade(lastDigit);
     ...
     */
    private shouldPlaceTrade(lastDigit: number): boolean {
    ...
    private async placeTrade(): Promise<void> {
        ...
        try {
            ...
            // Buy the contract
            const buyResponse = await derivAPIService.buyContract(
    ...
    private async getProposal(): Promise<any> {
        ...
        const contractType = this.config.tradeType === 'OVER' 
            ? 'DIGITOVER' 
            : 'DIGITUNDER';
