...
class DerivConnectionPoolService {
    ...
    private readonly appIdConfigs: AppIdConfig[] = [
        {
            appId: '67649',  // Your default App ID - UNCHANGED
            type: ConnectionType.CORE,
            description: 'Core Trading Features',
            features: ['Bot Builder', 'Main Trading', 'Account Management', 'Core API'],
    ...
    private initializeConnections(): void {
        ...
        console.log('✅ Connection Pool initialized with 4 App IDs');
    ...
    public getConnection(type: ConnectionType): RobustDerivConnectionService {
        ...
        if (!connection) {
            console.warn(`⚠️ Connection type ${type} not found, falling back to CORE`);
            return this.connections.get(ConnectionType.CORE)!;
        }
        return connection;
    }
    ...
    public async subscribeToTicks(
        ...
        console.log(`📡 Subscribing to ${symbol} via ${connectionType.toUpperCase()} (${this.getAppId(connectionType)})`);
    ...
    public async forceReconnectAll(): Promise<void> {
        ...
        const reconnectPromises = Array.from(this.connections.values()).map(connection => 
    ...
    public startHealthMonitoring(): void {
        setInterval(() => {
            ...
            console.log(`💓 Health: ${health.activeConnections}/${health.totalConnections} connections active, ${health.totalSubscriptions} total subscriptions`);
        }, 60000); // Every minute
    }
